...
const Dashboard = () => {
  ...
  const Sidebar = () => <div className={`
      ${isMobile ? `fixed inset-y-0 left-0 z-50 w-80 transform transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}` : 'w-80 flex-shrink-0'} 
    ...
    `}>
      {isMobile && <div className="absolute top-4 right-4 z-10">
          <Button variant="ghost" size="sm" onClick={() => setSidebarOpen(false)} className="text-purple-600 hover:text-purple-700 hover:bg-purple-100/50">
            <X className="h-5 w-5" />
          </Button>
        </div>}
      ...
      <div className="p-4 space-y-6 h-full overflow-y-auto">
        ...
        {/* User Profile */}
        <Card className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm border-purple-200/50 dark:border-purple-800/30 shadow-lg hover:shadow-xl transition-all duration-300 hover-lift">
          ...
          <CardContent>
            <div className="flex items-center space-x-4">
              <Avatar className="h-12 w-12 ring-2 ring-purple-200 dark:ring-purple-700 shadow-md">
            ...
            <Button variant="outline" className="mt-4 w-full border-purple-200 text-purple-700 hover:bg-purple-50 hover:border-purple-300 dark:border-purple-700 dark:text-purple-300 dark:hover:bg-purple-900/30 transition-all duration-200" onClick={logout}>
              Sign Out
            </Button>
          </CardContent>
        </Card>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            {navigationItems.map(item => <Button key={item.id} variant={activeTab === item.id ? "default" : "ghost"} className={`w-full justify-start transition-all duration-200 ${activeTab === item.id ? "bg-purple-gradient text-white shadow-lg hover:shadow-xl transform hover:scale-[1.02]" : "text-purple-700 dark:text-purple-300 hover:bg-purple-100/70 dark:hover:bg-purple-900/30 hover:text-purple-800 dark:hover:text-purple-200"}`} onClick={() => {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-purple-100 dark:from-gray-900 dark:via-purple-900/10 dark:to-gray-800">
      {/* Mobile overlay */}
      {isMobile && sidebarOpen && <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-40" onClick={() => setSidebarOpen(false)} />}
      ...
      <div className="flex min-h-screen">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {/* Mobile Header */}
          {isMobile && <header className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-b border-purple-200/50 dark:border-purple-800/30 p-4 sticky top-0 z-30">
              <div className="flex items-center justify-between">
                <Button variant="ghost" size="sm" onClick={() => setSidebarOpen(true)} className="text-purple-600 hover:text-purple-700 hover:bg-purple-100/50">
            ...
            <div className="max-w-7xl mx-auto">
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                ...
                <div className="sm:hidden mb-4">
                  <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm border border-purple-200/50 dark:border-purple-800/30 rounded-lg p-2">
